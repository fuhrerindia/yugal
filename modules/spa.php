...
class YugalSPA
{
    ...
    public function page($arr)
    {
        ...
        $fallback = isset($arr['fallback']) ? $this->removeHtmlComments_($this->replacePathWihKeyword($arr['fallback'])) : "";
        ...
        $isFallback = $fallback === "" ? false : true;
        $isFallback_str = $fallback === "" ? 'false' : 'true';

        $js_head = $this->mergeComponentVariable($isFallback ? "" : $head);
        $js_body = $this->mergeComponentVariable($isFallback ? $fallback : $body);
        $js_willm = $this->replace_backticks($willmount);
        $jsdidm = $this->replace_backticks($didmount);
        $js_style = $isFallback ? "" : $style;
...
HTML;
        ...
        <script src="{$this->_project_root}/modules/chunk.js" defer></script>        
HTML) : "";
